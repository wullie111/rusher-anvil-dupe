...
public class AutoAnvilRenameModule extends ToggleableModule {
	...
	private Integer targetShulkerSlot = 7;
	...
	private final NumberSetting<Integer> clickDelay = new NumberSetting<>("Click Delay", 1, 0, 29);
	private int delay = 0;
	...
	void Name(){
		...
		if (renameCount>=5){
		 ...
		 // Place the item in the input slot
	}
	...
	void tick() {
		...
		if (!itemStackOutput.isEmpty()
			...
			Name();
			InventoryUtils.clickSlot(7, false);
